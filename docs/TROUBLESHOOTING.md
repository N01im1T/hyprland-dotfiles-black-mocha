...
- `python3: No module named black_mocha`: run `git pull --ff-only` and rerun the installer with the same options. The deployment step replaces the incomplete runtime directory with the correctly nested Python package; uninstalling first is not required.
- SDDM still shows its blue embedded theme: update the repository and rerun the installer. Black Mocha is installed as the last local drop-in, and an existing higher-priority `Current=` entry in `/etc/sddm.conf` is updated as well. Reboot after installation; no uninstall is needed.
