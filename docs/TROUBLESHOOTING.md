...
- `paru: error while loading shared libraries: libalpm.so.*`: rerun `./install.sh`. The installer detects an AUR helper broken by a pacman/libalpm ABI upgrade and force-reinstalls `paru-bin` before continuing. Use `--no-aur` if you want to finish without optional AUR applications.
